                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('users.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="id_pertanyaan" class="block text-sm font-medium text-gray-700">Pertanyaan
                                    Keamanan</label>
                                <select name="id_pertanyaan" id="id_pertanyaan"
                                    ...
                                    @foreach ($pertanyaans as $pertanyaan)
                                        <option value="{{ $pertanyaan->id }}"
                                            {{ old('id_pertanyaan') == $pertanyaan->pertanyaan ? 'selected' : '' }}>
                                            {{ ucfirst($pertanyaan->pertanyaan) }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('id_pertanyaan')
                            ...
                            <div>
                                <label for="jawaban" class="block text-sm font-medium text-gray-700">Jawaban Pertanyaan
                                    Keamanan</label>
                                <input type="text" name="jawaban" id="jawaban"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('jawaban') border-red-500 @enderror"
